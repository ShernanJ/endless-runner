...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Physics.CheckSphere(c.position, 0.1f, groundLayer, QueryTriggerInteraction.Ignore))
            {
                isBlocked = true;
                if (!blockAudioPlayed)
                {
        ...
        if (isGrounded && (jumpPressed || jumpTimer > 0 && Time.time < jumpTimer + jumpPeriod) && !isUnderCeiling && !isSliding)
        {
